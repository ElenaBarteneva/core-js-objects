  ...
  return objects.reduce((result, obj) => {
    ...
    if (Object.entries(obj)) {
      Object.entries(obj).forEach((entry) => {
        accObj[entry[0]] = (accObj[entry[0]] ?? 0) + entry[1];
      });
    }
    return accObj;
  }, {});
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    if (queue[i] === 25) {
      cost += 25;
    } else if (queue[i] - 25 > cost) return false;
  }
  return true;
}
...
class SelectorBuilder {
  ...
  element(value) {
    ...
    cssSelectorBuilder.selector = this.selector + value;
    cssSelectorBuilder.components = [...this.components];
    cssSelectorBuilder.components.push('element');
  ...
  id(value) {
    ...
    cssSelectorBuilder.selector = `${this.selector}#${value}`;
    cssSelectorBuilder.components = [...this.components];
    cssSelectorBuilder.components.push('id');
    return cssSelectorBuilder;
  }
  ...
  class(value) {
    ...
    cssSelectorBuilder.selector = `${this.selector}.${value}`;
    cssSelectorBuilder.components = [...this.components];
    cssSelectorBuilder.components.push('class');
    return cssSelectorBuilder;
  }
  ...
  attr(value) {
    ...
    cssSelectorBuilder.selector = `${this.selector}[${value}]`;
    cssSelectorBuilder.components = [...this.components];
    cssSelectorBuilder.components.push('attribute');
    return cssSelectorBuilder;
  }
  ...
  pseudoClass(value) {
    ...
    cssSelectorBuilder.selector = `${this.selector}:${value}`;
    cssSelectorBuilder.components = [...this.components];
    cssSelectorBuilder.components.push('pseudo-class');
    return cssSelectorBuilder;
  }
  ...
  pseudoElement(value) {
    ...
    cssSelectorBuilder.selector = `${this.selector}::${value}`;
    cssSelectorBuilder.components = [...this.components];
    cssSelectorBuilder.components.push('pseudo-element');
    return cssSelectorBuilder;
  }
  ...
  combine(selector1, combinator, selector2) {
    ...
    const cssSelectorBuilder = new SelectorBuilder();
    cssSelectorBuilder.selector = `${selector1.selector} ${combinator} ${selector2.selector}`;
    return cssSelectorBuilder;
  }
